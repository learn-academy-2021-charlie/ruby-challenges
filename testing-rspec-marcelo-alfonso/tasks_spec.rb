...
describe 'Task' do 
    ...
    it 'has a title we can retrieve' do 
        #Arrange
        task_one = Task.new
        task_one.title = "New ToDo"

        # expect{ task_one.title = "New Todo"}.to_not raise_error
        expect(task_one.title).to be_a String
        expect{task_one.title}.to_not raise_error
        # we don't want to check to see if the string is a specific string, just if we can get it and it's a string
        # expect(task_one.title).to eq "New ToDo"
    end
